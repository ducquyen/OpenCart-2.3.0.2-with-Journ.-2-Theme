...
class ControllerExtensionPaymentBoletobb extends Controller {
	public function index() {
        ...
        $this->language->load('extension/payment/boleto_bb');
        ...
        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/extension/payment/boleto_bb.tpl')) {
            return $this->load->view($this->config->get('config_template') . '/template/extension/payment/boleto_bb.tpl', $data);
        } else {
            return $this->load->view('default/template/extension/payment/boleto_bb.tpl', $data);
        }
	...
	public function confirm() {
		...
		if ($this->session->data['payment_method']['code'] == 'boleto_bb') {
			$this->load->model('checkout/order');

			$this->model_checkout_order->addOrderHistory($this->session->data['order_id'], $this->config->get('boleto_bb_order_status_id'));
		}
	}
    ...
    public function callback() {
        ...
        if($order_info){
			...
			$dadosboleto["nosso_numero"] = str_pad($order_info['order_id'], 8, "0", STR_PAD_LEFT); //$this->config->get('boleto_bb_nosso_numero');
			...
			$dadosboleto["demonstrativo1"] = $this->config->get('boleto_bb_demonstrativo1').$taxa;
			$dadosboleto["demonstrativo2"] = $this->config->get('boleto_bb_demonstrativo2');
			...
			*/
			...
			$output = include("boleto/include/layout_bb.php");

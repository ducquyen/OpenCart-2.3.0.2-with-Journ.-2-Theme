<?php
class ModelExtensionPaymentBoletobb extends Model {
	public function getMethod($address, $total) {
		...
		if ($this->config->get('boleto_bb_total') > 0 && $this->config->get('boleto_bb_total') > $total) {
			$status = false;
		} elseif (!$this->config->get('boleto_bb_geo_zone_id')) {
			$status = true;
